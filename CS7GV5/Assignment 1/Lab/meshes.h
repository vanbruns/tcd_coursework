...
private:
	...
	int specularExponentLocation = 0;
	int kLocation = 0;
protected:
	Shader* pShader;
...
public:
	...
	virtual void init(Shader* shader, LightSource* lightSource) {
		...
		if (!ignoreSurfaceAndLighting) {
			...
			specularExponentLocation = glGetUniformLocation(pShader->getShaderID(), "specularExponent");
			kLocation = glGetUniformLocation(pShader->getShaderID(), "k");
		}
...
class AirplaneMesh : public GenericMesh {
public:
	void init(Shader* shader, LightSource* lightSource);
};

class PropellerMesh : public GenericMesh {
public:
	void init(Shader* shader, LightSource* lightSource);
};

class WheelsMesh : public GenericMesh {
public:
	void init(Shader* shader, LightSource* lightSource);
};

class Landscape : public GenericMesh {
public:
	void init(Shader* shader, LightSource* lightSource);
};